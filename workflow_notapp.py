...
def label_inter_number(gdf_exploded):
    """
    1. operate intersection between boundary_line and Polygon & LineString
    ...
    def report(result):
        ...
        result_copy["has_interior"] = pd.to_numeric(result_copy["has_interior"], errors='coerce')
        has_interior = result_copy.index[result_copy["has_interior"].fillna(False)]
        has_interior_list = has_interior.tolist() # e.g.[0,0,5,45....]
        print ("---------index(level-0) of polygon has interior----------")
        print(has_interior_list)

        result_copy['exterior_inter'] = pd.to_numeric(result_copy['exterior_inter'], errors='coerce')
        exterior_has_inter = result_copy.index[result_copy["exterior_inter"].fillna(0) > 0].tolist()
        print("-----------Exterior has intersection with boundary: Index(level-0) ----------")
        print(exterior_has_inter)

        result_copy['interior_inter'] = pd.to_numeric(result_copy['interior_inter'], errors='coerce')
        interior_has_inter = result_copy.index[result_copy["interior_inter"].apply(
        ...
        result_copy['line_inter'] = pd.to_numeric(result_copy['line_inter'], errors='coerce')
        line_has_inter = result_copy.index[result_copy["line_inter"].fillna(0)>0].tolist()
...
def regroup(gdf_processed):
    ...
    def geom_to_polys_lines(geom):
        ...
        if geom.geom_type in ["Polygon", "LineString", "Point"]:
            return [geom]
        if geom.geom_type in ["MultiPolygon", "MultiLineString","MultiPoint"]:
